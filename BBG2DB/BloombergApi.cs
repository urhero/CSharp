...
namespace BloombergConsole
{
    class BloombergApi
    {
        ...
        internal Dictionary<string,     //Security 
            ...
            Dictionary<string, object>> //Fields and values 
            > GetSecuritiesFieldsByDate(string[] securities, string[] fields, string[,] options)  //, DateTime startDate, DateTime endDate)
        {
            ...
            foreach (var security in securities)
            {
                if (security.Substring(security.Length - 5, 5) != "INDEX" || security.Length < 5)
                    securitiesElement.AppendValue(security + " EQUITY");
                else securitiesElement.AppendValue(security);
            }
            ...
            for (i = 0; i < options.Length / 2; i++)
                ...
                {
                    if (options[i, 1].ToUpper() == "TRUE" || options[i, 1].ToUpper() == "T")
                        historyDataRequest.Set(options[i, 0], true);
                    else if (options[i, 1].ToUpper() == "FALSE" || options[i, 1].ToUpper() == "F")
                        historyDataRequest.Set(options[i, 0], false);
                    else
                        historyDataRequest.Set(options[i, 0], options[i, 1]);
                }
            ...
            while (!done)
            {
                    ...
                    eventObj.Type == Event.EventType.PARTIAL_RESPONSE)
                {
                    ...
                    foreach (Message msg in eventObj)
                    {
                        ...
                        for (i = 0; i < fieldData.NumValues; i++)
                        {
